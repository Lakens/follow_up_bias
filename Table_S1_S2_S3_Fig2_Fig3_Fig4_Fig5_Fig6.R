...
nrconditions=length(conditionselection)
...
# 5. Summarizing the simulations (Table S1, S2, S3) ----
...
print(results)
write.table(results, paste("results_",toString(conditionselection),".txt",sep=""))
...
p1 <- eval(bquote(ggplot(data=plotdata, aes(factor(condition),ES, fill = factor(plotdata$EffectSize))))) + 
  ...
  theme(text=element_text(size=18), panel.border = element_rect(colour = "black", fill=NA, size=1), axis.text.x = element_blank(), 
        ...
        strip.text.x = element_text(size = 16), strip.background = element_rect(fill = 'white'), legend.text.align = 0, legend.text = element_text(size = rel(1.0))) +
  ...
  ylab("Effect Size Estimate")  + xlab(paste("Groups = ",Kselection,", ES = ",ESselection,", Power = ",powerselection,sep=""))
p1
dev.off()
...
p2 <- ggplot(data=plotdata, aes(factor(condition),SS, fill = factor(EffectSize))) + 
  geom_boxplot(outlier.colour=alpha("white", 0.05)) + scale_fill_manual(values=cbbPalette, name = "Effect Size", labels=c(parse(text="epsilon^2"), parse(text="eta^2"), parse(text="omega^2")), breaks=c("epsilon","eta","omega")) +
  facet_grid(.~NPilot, scales="free", space="free") +
  geom_violin(alpha=0.5) +
  theme(text=element_text(size=18), panel.border = element_rect(colour = "black", fill=NA, size=1), axis.text.x = element_blank(), 
        axis.ticks.x = element_blank(), panel.grid.major.x = element_blank(), 
        panel.background = element_rect(fill = "white"), strip.text.x = element_text(size = 16), strip.background = element_rect(fill = 'white'), legend.text.align = 0, legend.text = element_text(size = rel(1.0))) +
...
p3 <- ggplot(results, aes(x=names, y=obspower2, group=1, fill = factor(ESmeasure))) + 
  ...
  theme(text=element_text(size=18), panel.border = element_rect(colour = "black", fill=NA, size=1), axis.text.x = element_blank(), 
        axis.ticks.x = element_blank(), panel.grid.major.x = element_blank(), panel.background = element_rect(fill = "white"), 
        strip.text.x = element_text(size = 16), strip.background = element_rect(fill = 'white'), legend.text.align = 0, legend.text = element_text(size = rel(1.0))) +
  geom_hline(yintercept=(100*unique(results$power)), colour="gray20", linetype="dashed", size = 1.4) +
