...
tiff(file="ESmat3.tiff",width=7200,height=5000, units = "px", res = 300)
plot.new()
ggplot(data=ESs, aes(x=ES, y=value, fill = ES)) + 
  geom_violin() + scale_fill_manual(values=cbbPalette, name = "Effect Size", labels=c(parse(text="epsilon^2"), parse(text="eta^2"), parse(text="omega^2")), breaks=c("2","1","3"), guide = guide_legend(direction = "vertical", label.position = "right", label.hjust = 0.3)) +
  ylab("effect size") +
